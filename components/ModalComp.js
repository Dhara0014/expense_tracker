  ...
  export default function ModalComp({ isModalOpen, setIsModalOpen, createCategory, fetchCategories}) {
    const [newCategory, setNewCategory] = useState("");
    if (!isModalOpen) return null;

    const addCategory = async() => {
      if (newCategory.trim()) {
        await createCategory({name: newCategory, icon: "FaArrowsTurnToDots", isDelete: true});
