...
import { iconMaps } from '@/app/components/helper/Icons';
import { getLocalStorage, setLocalStorage } from '@/app/components/helper/Local';
import expenceSchema from '@/app/components/errorHandler/validationSchema';


const page = () => {
    const {id} = useParams();
    const defaultValues = getLocalStorage("list")?.filter(itm => itm.id == id)[0];
    const route = useRouter();
    const [type, setType] = useState(defaultValues?.type ?? "");
    const [categories, setCategories] = useState([
        { name: "Groceries", isDelete: false, icon: "FaShoppingBag" },
        { name: "Milk", isDelete: false, icon: "GiMilkCarton" },
        { name: "Vegetables/Fruits", isDelete: false, icon: "GiFruitBowl" },
        { name: "Petrol", isDelete: false, icon: "BsFillFuelPumpFill" },
        { name: "Shopping", isDelete: false, icon: "RiShoppingCart2Fill" },
        { name: "Cloths", isDelete: false, icon: "GiClothes" },
    ]);
    const [selectedCategory, setSelectedCategory] = useState(defaultValues?.selectedCategory ?? null);
    ...
    const [paymentMethod, setPaymentMethod] = useState(defaultValues?.paymentMethod ?? null);
    ...
    const submitHandler = (e) => {
        ...
        let res = getLocalStorage("list").map(itm => {
            if(itm.id == id){
               return {id, ...formData } 
            }
            return itm
    ...
    return (
        <MainLayout>
            <form onSubmit={submitHandler} className='gap-[10 10]' >
                ...
                {
                    erorrs?.type && <p className="text-red-500 text-sm mt-1" >{erorrs?.type}</p>
                }
                <div>
